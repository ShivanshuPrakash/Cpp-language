...
// Special Syntax
/*
o)  C++ supports a special syntax for passing arguments to multiple base classes
o)  The constructor of the derived class receives all the arguments at once and then will pass  the call to the respective base classes
o)  The body is called after the constructors is finished executing
...
o). The constructors for virtual base classes are invoked before a non-virtual base class
o). If there are multiple virtual base classes, they are invoked in the order declared
o). Any non-virtual base class are then constructed before the derived class constructor is executed
